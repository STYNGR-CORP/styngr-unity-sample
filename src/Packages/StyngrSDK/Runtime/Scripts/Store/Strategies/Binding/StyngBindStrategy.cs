...
{
    /// <summary>
    /// Binding strategy for the Styngs.
    /// </summary>
    internal class StyngBindStrategy : IBindStrategy
    {
        ...
        /// <summary>
        /// Creates an instance of the <see cref="StyngBindStrategy"/> class.
        /// </summary>
        /// <param name="bindingFinished">Action which will be invoked when the binding process has been finished.</param>
        public StyngBindStrategy(Action bindingFinished)
        {
        ...
        public IEnumerator Bind(GameEvent eventData, IProduct product)
        {
            ...
            if (isProductSet)
            {
                if (hasBoundProduct && !gameEventData.AreProductIdsEqual(tempProduct.GetId()))
                {
                    ...
                    yield return StoreManager.Instance.StoreInstance.BindEventAsync(ProductType.Styng, tempProduct, gameEventData, OnBindSuccess, OnFail);
                }
                if (!hasBoundProduct)
                {
                    yield return StoreManager.Instance.StoreInstance.BindEventAsync(ProductType.Styng, tempProduct, gameEventData, OnBindSuccess, OnFail);
        ...
        private IEnumerator BindPrivate(Tuple<GameEvent, bool> eventsToToggleStates, IProduct product)
        {
            gameEventData = eventsToToggleStates.Item1;
            tempProduct = product;

            var hasBoundProduct = gameEventData.TryGetBoundProduct(out var boundProduct);
            var isEventSet = eventsToToggleStates.Item2;
            ...
            {
                // If there is a bound product that is the same as currently bound product
                if (hasBoundProduct && gameEventData.AreProductIdsEqual(tempProduct.GetId()))
                {
                    yield return StoreManager.Instance.StoreInstance.UnbindEventAsync(gameEventData, OnUnbindSuccess, OnFail);
                }
        ...
        {
            var tempEvent = new StyngGameEvent(gameEventData);
